...
namespace EPT_Data_Acquistion
{
    public partial class EPT_Data_Acquistion : Form
    {
        ...
        //Buffer to hold bytes transferred from the UnoProLogic2
...
*/
        ...
        //Cursor Position Offset values
        public float Cursor1Position;
        public float Cursor1OffsetVoltage;
        public float Cursor2Position;
        public float Cursor2OffsetVoltage;
        ...
        public int TriggerChannelSelection = 0;
        public const int TriggerChannelOne = 1;
        public const int TriggerChannelTwo = 2;
        public const int TriggerChannelThree = 3;
        public const int TriggerChannelFour = 4;
        public int TriggerValueThreshold = 0;
        public bool TriggerOnDetected = false;
        public bool TriggerOnDisplayFlag = false;
        public uint TriggerOnDetectedCounter = 0;
        public bool TriggerOffDetected = false;
        public uint TriggerOffDetectedCounter = 0;
        public bool TriggerOnStorage = false;
        public int TriggerIndexOffset = 0;
        public double TriggerVoltageScaleFactor = 0.0048828;

        //FOR TEST ONLY
        //Streamwrite file
        string ChosenRecieveFile = "TriggerTestLogFile.txt";
        TextWriter tw;

        //Flag to determine if Textwriter is open
        bool twOpen = false;
        ...
        {
            //Set the state of the Open and Close buttons
            btnOpenDevice.Enabled = true;
            btnCloseDevice.Enabled = false;

            //Set the initial state of the Cursor Labels
        ...
        private void btnOpenDevice_Click(object sender, EventArgs e)
        {
            ...
            tw = new StreamWriter(ChosenRecieveFile);

            // write a line of text to the file
            tw.WriteLine("Trigger Debug Start File\n");

            // Set the streamwrite flag
            twOpen = true;
        ...
        private void RefreshScope(Graphics g)
        {
            ...
            try
            {
                //ScopeIndex = GetScopeBufferIndex(0, 0, TriggerIndexOffset);
                //if (twOpen)
                //    tw.WriteLine("Invalidate() called at ScopeIndex: " + ScopeIndex + "\n");
                for (int OuterDimension = 0; OuterDimension < 4; OuterDimension++)
                {
                    xlast = -1;
                    ylast = 0;
                    xDraw = 0;
                    ...
                    //Get the value of ScopeIndex from GetScopeBufferIndex()
                    ScopeIndex = GetScopeBufferIndex(OuterDimension, IndexOffset, TriggerIndexOffset);
        ...
        public void DisplayChannel2Setup()
        {
            ...
            this.Invoke(new MethodInvoker(delegate() { cmbTriggerChanSelect.Items.Add((ScopeBufferChannelSelect + 1)); })); 
            ...
            this.Invoke(new MethodInvoker(delegate() { Channel_2_VerticalLabel.Location = new Point(480, 225); }));
        ...
        public void DisplayChannel3Setup()
        {
            ...
            this.Invoke(new MethodInvoker(delegate() { Channel_3_VerticalLabel.Location = new Point(480, 225); }));
        ...
        public void DisplayChannel4Setup()
        {
            ...
            this.Invoke(new MethodInvoker(delegate() { Channel_4_VerticalLabel.Location = new Point(480, 225); }));
        ...
        public void ResumeOscilloscope()
        {
            //Start ADC Conversion Continuous
            //EPT_AH_SendTrigger((byte)4);

            //Continue the Display update
            TriggerOnDisplayFlag = false;
        }

        public void PauseOscilloscope()
        {

            //Stop ADC Conversions
            //EPT_AH_SendTrigger((byte)8);

            //Stop updating the Display while
            //a trigger is active
            TriggerOnDisplayFlag = true;


        }

        public void ResumeUnoProLogic2()
        {
            //Start ADC Conversion Continuous
            //EPT_AH_SendTrigger((byte)4);
        ...
        private void trkBrTimeBase_Scroll(object sender, EventArgs e)
        {
            ...
            switch (trkBrTimeBase.Value)
            {
                case 1:                               //Sample Rate:
                    ...
                    ADCConversionDelayByte[1] = 0x00; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //250us
                    lblTimeBase.Text = "250 us";
                    TimeBaseFactor = 0.25;
                    break;
                case 2:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0x21; //24'h21; //0.010ms
                    ADCConversionDelayByte[1] = 0x01; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //500us
                    lblTimeBase.Text = "500 us";
                    TimeBaseFactor = 0.50;
                    break;
                case 3:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0xd0; //24'h3de; //0.034ms
                    ADCConversionDelayByte[1] = 0x03; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //1.7 ms
                    lblTimeBase.Text = "1.7 ms";
                    TimeBaseFactor = 1.70;
                    break;
                case 4:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0x10; //24'h0910; //0.055ms
                    ADCConversionDelayByte[1] = 0x09; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //2.75 ms
                    lblTimeBase.Text = "2.75 ms";
                    TimeBaseFactor = 2.75;
                    break;
                case 5:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0xe4; //24'hce4; //0.07ms
                    ADCConversionDelayByte[1] = 0x0c; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //3.5 ms
                    lblTimeBase.Text = "3.5 ms";
                    TimeBaseFactor = 3.50;
                    break;
                case 6:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0xc8; //24'h19c8; //0.12ms
                    ADCConversionDelayByte[1] = 0x19; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //6 ms
                    lblTimeBase.Text = "6 ms";
                    TimeBaseFactor = 6.00;
                    break;
                case 7:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0xe8; //24'h80e8; //0.5ms
                    ADCConversionDelayByte[1] = 0x80; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //25 ms
                    lblTimeBase.Text = "25 ms";
                    TimeBaseFactor = 25.00;
                    break;
                case 8:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0xd0; //24'ha0d0; //0.65ms
                    ADCConversionDelayByte[1] = 0xa0; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x001; //32.5 ms
                    lblTimeBase.Text = "32.5 ms";
                    TimeBaseFactor = 32.50;
                    break;
                case 9:                               //Sample Rate:
                    ADCConversionDelayByte[0] = 0x10; //24'h50910; //0.7ms
                    ADCConversionDelayByte[1] = 0xc0; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //35 ms
                    lblTimeBase.Text = "35 ms";
                    TimeBaseFactor = 35.00;
                    break;
                case 10:                              //Sample Rate:
                    ADCConversionDelayByte[0] = 0xff; //24'haffff; //1ms
                    ADCConversionDelayByte[1] = 0xff; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //50 ms
                    lblTimeBase.Text = "50 ms";
                    TimeBaseFactor = 50.00;
                    break;
                default:                              //Sample Rate:
                    ADCConversionDelayByte[0] = 0x20; //24'ha1220; //10ms
                    ADCConversionDelayByte[1] = 0x12; //TimeBase per Interval:
                    ADCConversionDelayByte[2] = 0x00; //625 ms
                    lblTimeBase.Text = "625 ms";
                    TimeBaseFactor = 625.00;
                    break;
            ...
            //Update the Time Scale on the Display
            UpdateTimingScale(TimeBaseFactor);
